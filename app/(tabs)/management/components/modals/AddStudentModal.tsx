...
const AddStudentModal = ({ visible, onClose, setStudents }:any) => {
  ...
  const handleSave =async () => {
    ...
    // if(formData.isMinor && !formData.parentName || !formData.parentPhone){
    //   alert('Please fill in parent details for minors');
    //   return;
    // }
    const token = await SecureStore.getItemAsync('token');
    try {
      const response = await axios.post('http://10.182.90.139:3000/students/add',formData,{
        headers: {
      ...
      if(response.status===200){
        ...
        setStudents((prev:any)=>[...prev,response.data])
        onClose();
      }else{
